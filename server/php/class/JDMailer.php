...
class JDMailer
{
	...
	static function sendMail($to, $subject, $content) {
		foreach (["g_smtp_host", "g_smtp_port", "g_from_email", "g_from_name"] as $e) {
			if (! isset($GLOBALS[$e]))
		...
		if (@$GLOBALS["g_smtp_debug"])
			$mail->SMTPDebug = 1;

		if (isset($GLOBALS["g_smtp_username"])) {
		...
		logit("sendmail from " . $GLOBALS["g_from_email"] . " to $to: " . $mail->Subject);
		if(!$mail->send()) {
